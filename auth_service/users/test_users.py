...
from auth_service.users.models import (
    CustomUser,
    UserPreferences,
    UserRole,
    UserSession,
...
class DatabaseIntegrationTestCase(TransactionTestCase):
    ...
    def test_user_model_relationships(self):
        ...
        # Create preferences
        preferences = UserPreferences.objects.create(
            user=user, language="es", theme="dark"
        )

        # Create role
        role = UserRole.objects.create(user=user, role_name="customer")
    ...
    def test_database_tables_exist(self):
        ...
        with connection.cursor() as cursor:
            ...
            self.assertGreater(
                len(found_tables),
                0,
                f"Expected tables not found. Available tables: {table_names}",
            )
...
class UserSessionModelTestCase(TestCase):
    ...
    def test_session_expiration(self):
        ...
        expires_at = timezone.now() - timezone.timedelta(hours=1)
        session = UserSession.objects.create(
            user=self.user, session_token="expired_token", expires_at=expires_at
        )
    ...
    def test_session_deactivation(self):
        ...
        expires_at = timezone.now() + timezone.timedelta(hours=1)
        session = UserSession.objects.create(
            user=self.user, session_token="active_token", expires_at=expires_at
        )
...
class UserRoleModelTestCase(TestCase):
    ...
    def test_role_creation(self):
        """Test role creation"""
        role = UserRole.objects.create(user=self.user, role_name="customer")
